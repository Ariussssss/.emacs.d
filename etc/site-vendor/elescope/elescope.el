...
(defun elescope--github-clone (entry-str)
  "Clone the GitHub project designated by ENTRY."
  (let* ((entry (get-text-property 0 'full-entry entry-str))
	...
	(progn
	  (setq clone-url (format "git@github.com:%s.git" entry-str)) 
	  (setq path entry-str)
	  )
      )
    (if path
	...
	(if (file-directory-p destination)
	  ...
	  (if (eql 0 (arz/async-shell-callback command
					       (lambda ()
						 (find-file destination))))
